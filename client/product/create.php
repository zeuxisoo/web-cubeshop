...
if ($action == "create" || $action == "update") {
	...
	}elseif ($cube_id != $client['id']) {
		Session::set("error", "Can not make a create or update on this product");
	}
...
if ($action == "create") {
	...
	if (empty($id) === true) {
		...
		// If the exists product is not client
		if ($row['cube_id'] != $client['id']) {
	...
	//
	$records = Table::fetch_all("cubes", array(
		"select" => "id, cube_name", 
		"where" => array(
			"id" => $client['id']
		)
